...
function loadSyntestJS(config = {}) {
  let syntestjs;
  let coverageConfig;
  let log = config.logger ? config.logger.log : console.log;
  let ui = new PluginUI(log);

  // Handle --syntestjs flag
  config.syntestjs
    ? (syntestjs = path.join(config.workingDir, config.syntestjs))
    : (syntestjs = path.join(config.workingDir, ".syntest.js"));

  // Catch syntestjs syntax errors
  if (shell.test("-e", syntestjs)) {
    try {
      coverageConfig = require(syntestjs);
    } catch (error) {
      error.message = ui.generate("syntestjs-fail") + error.message;
  ...
  if (coverageConfig.mocha && typeof coverageConfig.mocha === "object") {
    config.mocha = Object.assign(config.mocha, coverageConfig.mocha);
  }
