const UI = require('../../lib/instrumentation/ui').UI;
...
class PluginUI extends UI {
  ...
  report(kind, args=[]){
    ...
    const kinds = {
      ...
      'id-clash': `${w}  ${c.red("The 'network_id' values in your truffle network ")}` +
                        `${c.red("and .syntest.js are different. Using truffle's: ")} ${c.bold(args[0])}.\n`,
  ...
  generate(kind, args=[]){
    ...
    const kinds = {
      ...
      'syntestjs-fail': `${c.red('Could not load .syntest.js config file. ')}` +
