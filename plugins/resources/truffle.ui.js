...
class PluginUI extends UI {
  ...
  generate(kind, args = []) {
    ...
    const kinds = {
      ...
      "lib-fail":
        `${c.red("Unable to load plugin copy of Truffle library module. ")}` +
        `${c.red("Try installing Truffle >= v5.0.31 locally or globally.\n")}` +
        `Caught error message: ${args[0]}\n`,

      "syntestjs-fail":
        `${c.red("Could not load .syntest.js config file. ")}` +
        `${c.red("This can happen if it has a syntax error or ")}` +
        `${c.red("the path you specified for it is wrong.")}`,

      "tests-fail": `${x} ${c.bold(args[0])} ${c.red(
        "test(s) failed under coverage."
      )}`,

      "no-network":
        `${c.red("Network: ")} ${args[0]} ` +
