...
export class SolidityTarget extends Target {
    ...
    parseActions(): void {
        ...
        for (const key of Object.keys(fnMap)) {
            ...
            const args = fn.functionDefinition.slice(fn.functionDefinition.indexOf('(') + 1, fn.functionDefinition.indexOf(')')).split(',')
            const returnValue = fn.functionDefinition.slice(fn.functionDefinition.lastIndexOf('(') + 1, fn.functionDefinition.lastIndexOf(')')).split(' ')
