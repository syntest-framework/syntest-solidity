...
exports.setNetworkFrom = exports.setOuterConfigKeys = exports.setNetwork = void 0;
    ...
    if (config.network) {
        ...
        if (!isNaN(parseInt(network.network_id))) {
            ...
            if (api.providerOptions.network_id &&
                ...
                syntest_framework_1.getLogger().info("id-clash " + [parseInt(network.network_id)]);
            }
        ...
        if (!network.port) {
            syntest_framework_1.getLogger().info("no-port " + [api.port]);
            network.port = api.port;
        }
        // Warn: port conflicts
        if (api.port !== api.defaultPort && api.port !== network.port) {
            syntest_framework_1.getLogger().info("port-clash " + [network.port]);
        }
...
exports.setOuterConfigKeys = setOuterConfigKeys;
/**
...
exports.setNetworkFrom = setNetworkFrom;
