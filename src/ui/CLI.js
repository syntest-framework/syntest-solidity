...
class CLI extends UI_1.default {
    ...
    reportSkipped(config, skipped = []) {
        ...
        for (const item of skipped) {
            if (!started) {
                this.report("instr-skip", []);
                started = true;
            }
            this.report("instr-skipped", [item.relativePath]);
        }
    }
    report(text, args = []) {
        const c = chalk;
        ...
        const texts = {
            ...
            "id-clash": `${w}  ${c.red("The 'network_id' values in your truffle network ")}` + `${c.red("and .syntest.js are different. Using truffle's: ")} ${c.bold(args[0])}.\n`,
            "port-clash": `${w}  ${c.red("The 'port' values in your truffle network ")}` + `${c.red("and .syntest.js are different. Using truffle's: ")} ${c.bold(args[0])}.\n`,
        ...
        if (!this.silent) {
            syntest_framework_1.getLogger().info(texts[text]);
        }
    }
}
exports.default = CLI;
