 ...
 */
export class MappingStatement extends PrimitiveStatement<number[], Mapping> {
  ...
  copy() {
    return new MappingStatement(this.type, this.uniqueId, [...this.value]);
  }
  ...
  mutate(sampler: SoliditySampler, depth: number): Statement {
    if (prng.nextBoolean(sampler.deltaMutationProbability)) {
      ...
      newBytes[index] = Math.max(MappingStatement._lower_bound, newValue);
      newBytes[index] = Math.min(MappingStatement._upper_bound, newValue);

      return new MappingStatement(this.type, prng.uniqueId(), newBytes);
    } else {
