...
import { ControlFlowGraphFactory } from "../../analysis-solidity/lib/cfg/ControlFlowGraphFactory";
...
import {
  createTruffleConfig,
  getTestFilePaths,
  setupTempFolders as setupTemporaryFolders,
...
import { ASTGenerator } from "../../analysis-solidity/lib/ast/AbstractSyntaxTreeFactory";
...
export class SolidityLauncher extends Launcher<SolidityTestCase> {
  ...
  async initialize(): Promise<void> {
    ...
    await setupTemporaryFolders(this.tempArtifactsDir);
    ...
    const cfgGenerator = new ControlFlowGraphFactory();
  ...
  async preprocess(): Promise<void> {
    ...
    if (this.programState.rootContext.targets.length === 0) {
      // Shut server down
      LOGGER.error(
    ...
    for (const target of this.programState.rootContext.targets) this.userInterface.property(
  ...
  async process(): Promise<void> {
    ...
    for (const target of this.programState.rootContext.targets) {
      ...
      this.importsMap = new Map([
        ...this.importsMap.entries(),
        ...importMap.entries(),
      ]);
      this.dependencyMap = new Map([
        ...this.dependencyMap.entries(),
  ...
  async testTarget(
    ...
    try {
      ...
      if (error instanceof SolidityParser.ParserError) {
