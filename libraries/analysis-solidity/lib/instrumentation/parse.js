...
// Utilities
parse.configureStatementCoverage = function (value) {
  register.measureStatementCoverage = value;
};

parse.configureFunctionCoverage = function (value) {
  register.measureFunctionCoverage = value;
};
...
parse.ContractOrLibraryStatement = function (contract, expression) {
  ...
  if (expression.kind !== "interface") {
    ...
    if (expression.baseContracts && expression.baseContracts.length > 0) {
  ...
  if (expression.subNodes) {
    ...
    for (const construct of expression.subNodes) {
      parse[construct.type] && parse[construct.type](contract, construct);
    }
    // Unset flag...
...
parse.FunctionDefinition = function (contract, expression) {
  ...
  let temporaryContractName = contract.contractName;
  ...
  contract.contractName = temporaryContractName;
};
...
parse.SourceUnit = function (contract, expression) {
  for (const construct of expression.children) {
    parse[construct.type] && parse[construct.type](contract, construct);
  }
};
