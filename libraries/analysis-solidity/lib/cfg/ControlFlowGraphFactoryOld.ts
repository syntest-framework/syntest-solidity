...
import { SolidityArguments } from "../../../search-solidity/lib/SolidityLauncher";
 ...
 */
export class ControlFlowGraphFactory implements CFGFactory {
  ...
  removePlaceholder(cfg: CFG): void {
    ...
    for (const placeholderNode of cfg.nodes
      // Find all placeholder nodes
      .filter((n) => n.type === NodeType.Placeholder)) {
        for (const outgoingEdge of cfg.edges
          ...
          .filter((edge) => edge.from === placeholderNode.id)) {
            ...
            for (const incomingEdge of cfg.edges
